...
export async function createUploadIntent(input: {
  ...
  if (
    !ALLOWED_EVIDENCE_MIME_TYPES.includes(input.declaredMimeType) ||
    !Number.isInteger(input.declaredSizeBytes) ||
    input.declaredSizeBytes <= 0 ||
    input.declaredSizeBytes >= MAX_EVIDENCE_SIZE_BYTES
  )
  ...
  const intent = await db.transaction(async (tx) => {
    ...
    const [intent] = await tx
      .insert(paymentUploadIntents)
      .values({
        orderId: input.orderId,
        s3Key: "staging/" + randomUUID(),
