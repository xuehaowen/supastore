...
export async function verifyTrackingLink(input: { recoveryToken: string }) {
  return db.transaction(async (tx) => {
    const [consumed] = await tx
      .update(guestOrderSessions)
      .set({ recoveryUsedAt: new Date() })
      .where(
        and(
          eq(
            guestOrderSessions.recoveryTokenHash,
            hashToken(input.recoveryToken),
          ),
          isNull(guestOrderSessions.recoveryUsedAt),
    ...
    if (!consumed)
      throw new UnauthorizedError(
        "Invalid, expired, or already used recovery link.",
      );
