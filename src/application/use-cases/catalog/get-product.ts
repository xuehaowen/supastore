...
export async function getProduct(
  ...
  const [translationRows, variantRows, imageRows, categoryRows] =
    await Promise.all([
      db
        .select()
        .from(productTranslations)
        .where(and(eq(productTranslations.productId, product.id))),
      db
        .select({
  ...
  const translation =
    translationRows.find((t) => t.locale === locale) ??
    translationRows.find((t) => t.locale === settings?.defaultLocale);
